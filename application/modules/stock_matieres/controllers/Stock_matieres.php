	...
	public function listing()
	{
		...
		foreach ($resultat as $key) {
			...
			if($key->ID_STATUT_MATIERE ==1){
				$options .='<a class="dropdown-item" href="'.base_url("stock_matieres/Stock_matieres/index_update/".$key->ID_STOCK_MATIERE).'">
	...
	public function add()
	{
		...
		if ($this->form_validation->run() == FALSE){
			...
			if(!empty($donne_matiere)){
				...
				$this->Model->update('stock_matieres_premieres', ['ID_TYPE_MATIERE' => $ID_TYPE_MATIERE], array('QUANTITE_RECUE'=>($donne_matiere['QUANTITE_RECUE']+$QUANTITE_RECUE)));
