...
class Stock_Matieres_New extends MY_Controller {
...

	{
		...
		$data['lots']=$this->Model->getRequete('SELECT * FROM `stock_matieres_premieres` where IS_COMPLET=2 order by ID_STOCK_MATIERE');
	...
	$datas = [
		...
		'ID_STATUT_MATIERE' => 1,
		'ID_USER'           => $this->session->userdata('SUPERBAT_ID_USER'),
		'ID_COMPLET'           => $this->input->post('verify_complet'),
	];


	if (!empty($this->input->post('LOT_N'))) {
	$NUMERO_LOT =  $this->input->post('LOT_N');
	$one_value = $this->Model->getOne('stock_matieres_premieres',array('LOT_MP'=>$NUMERO_LOT));

	$datas = [
		'ID_TYPE_MATIERE'   => 33,
		'QUANTITE_COMMANDE' => $totalNet+$one_value['QUANTITE_COMMANDE'],
		'QUANTITE_RECUE'    => $totalNet+$one_value['QUANTITE_RECUE'],
		'LOT_MP'            => $NUMERO_LOT,
		'DATE_ENTREE'       => $date_entree,
		'ID_STATUT_MATIERE' => 1,
		'ID_USER'           => $this->session->userdata('SUPERBAT_ID_USER'),
	...
	$ID_STOCK_MATIERE = $this->Model->update('stock_matieres_premieres',array('ID_STOCK_MATIERE'=>$one_value['ID_STOCK_MATIERE']),$datas);
...
function liste_reception2(){
	$data['title']='Réception des matières premières';
	$commandes = $this->Model->getRequete("SELECT `ID_STOCK_MATIERE` as id, stock_matieres_premieres.`ID_TYPE_MATIERE`,stock_matieres_premieres.LOT_MP,type_matieres.DESCRIPTION,type_matieres.UNITE, `NBRE_COIlS`, `LONGEUR`, `COULEUR`, `QUANTITE_COMMANDE`, `QUANTITE_RECUE`, stock_matieres_premieres.`ID_FOURNISSEUR`,fournisseur.NOM,fournisseur.LOCALITE, stock_matieres_premieres.`ID_USER`,admin_user.NOM AS user,admin_user.PRENOM, `DATE_ENTREE`, `DATE_INSERT`,stock_matieres_premieres.ID_STATUT_MATIERE,statut_matieres.DESCRIPTION_STATUT_MATIERE,(SELECT COUNT(historique_stock_matieres_premieres.ID_STOCK_MATIERE) FROM historique_stock_matieres_premieres where historique_stock_matieres_premieres.ID_STOCK_MATIERE=id) as nombre_colis FROM `stock_matieres_premieres` JOIN type_matieres ON type_matieres.ID_TYPE_MATIERE=stock_matieres_premieres.ID_TYPE_MATIERE JOIN fournisseur ON fournisseur.ID_FOURNISSEUR JOIN admin_user ON admin_user.ID_USER=stock_matieres_premieres.ID_USER JOIN statut_matieres ON statut_matieres.ID_STATUT_MATIERE =stock_matieres_premieres.ID_STATUT_MATIERE WHERE stock_matieres_premieres.ID_STATUT_MATIERE!=2 and stock_matieres_premieres.LOT_MP IS NOT NULL");
	$data['commandes'] = $commandes;
	

	$this->load->view('Stock_Matieres_List_Reception_Intern_View',$data);
}




public function liste_affiche()
{

	$query_principal ="SELECT `ID_STOCK_MATIERE` as id, stock_matieres_premieres.`ID_TYPE_MATIERE`,stock_matieres_premieres.LOT_MP,type_matieres.DESCRIPTION,type_matieres.UNITE, `NBRE_COIlS`, `LONGEUR`, `COULEUR`, `QUANTITE_COMMANDE`, `QUANTITE_RECUE`, stock_matieres_premieres.`ID_FOURNISSEUR`,fournisseur.NOM,fournisseur.LOCALITE, stock_matieres_premieres.`ID_USER`,admin_user.NOM AS user,admin_user.PRENOM, `DATE_ENTREE`, `DATE_INSERT`,stock_matieres_premieres.ID_STATUT_MATIERE,statut_matieres.DESCRIPTION_STATUT_MATIERE,(SELECT COUNT(historique_stock_matieres_premieres.ID_STOCK_MATIERE) FROM historique_stock_matieres_premieres where historique_stock_matieres_premieres.ID_STOCK_MATIERE=id) as nombre_colis FROM `stock_matieres_premieres` JOIN type_matieres ON type_matieres.ID_TYPE_MATIERE=stock_matieres_premieres.ID_TYPE_MATIERE JOIN fournisseur ON fournisseur.ID_FOURNISSEUR JOIN admin_user ON admin_user.ID_USER=stock_matieres_premieres.ID_USER JOIN statut_matieres ON statut_matieres.ID_STATUT_MATIERE =stock_matieres_premieres.ID_STATUT_MATIERE WHERE stock_matieres_premieres.ID_STATUT_MATIERE!=2 and stock_matieres_premieres.LOT_MP IS NOT NULL";
	...
	$order_column = array('id','stock_matieres_premieres.LOT_MP', 'type_matieres.DESCRIPTION','DATE_ENTREE','QUANTITE_COMMANDE','QUANTITE_RECUE','admin_user.NOM','DATE_ENTREE');

	$order_by = isset($_POST['order']) ? ' ORDER BY ' . $order_column[$_POST['order']['0']['column']] . '  ' . $_POST['order']['0']['dir'] : ' ORDER BY id ASC';

	$search = !empty($_POST['search']['value']) ?
	"AND type_matieres.DESCRIPTION LIKE '%$var_search%' OR stock_matieres_premieres.LOT_MP LIKE '%$var_search%'  OR admin_user.NOM LIKE '%$var_search%' OR DATE_FORMAT(DATE_ENTREE, '%d/%m/%Y') LIKE '%$var_search%'  "
	: '';
	...
	foreach ($resultat as $key) {
        ...
        $row[] ='<span class="btn btn-primary">'. $key->LOT_MP .'</span>';
        $row[] = date("d/m/Y", strtotime($key->DATE_ENTREE));
		$row[] = $key->NOM;
...
public function listing_reception2()
{
