...
class Notifications
{
  ...
  //  }
   ...
   function newsend_mail($emailTo = array(), $subjet, $cc_emails = array(), $message, $attach = array()) {

    $config['protocol'] = 'smtp';
    $config['smtp_host'] = 'mail.socarvie.bi';
    $config['smtp_port'] = 465;
    $config['smtp_crypto'] = 'ssl';
    $config['smtp_auth'] = true;
    $config['smtp_user'] = 'dms_system@socarvie.bi';
    $config['smtp_pass'] = '@dms_system@';
    $config['mailtype'] = 'html';
    $config['charset'] = 'UTF-8';
    $config['wordwrap'] = TRUE;
    ...
    $this->CI->email->from('noreply@socarvie.bi', 'Document System Management');
    $this->CI->email->to($emailTo);
    if (!empty($cc_emails)) {
        foreach ($cc_emails as $key => $value) {
            $this->CI->email->cc($value);
